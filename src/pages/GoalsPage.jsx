...
import { Target, Flag, Car, Home, Plus, Briefcase, ChevronRight, X, Loader2 } from 'lucide-react';
...
export default function GoalsPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('goals_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'goals', filter: `user_id=eq.${userId}` }, () => load())
      .subscribe();
      ...
      {showForm && (
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input placeholder="Goal name" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm" />
            <input placeholder="Target amount (₹)" type="number" value={formData.target} onChange={e => setFormData(p => ({ ...p, target: e.target.value }))} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm" />
            <input placeholder="Current saved (₹)" type="number" value={formData.current} onChange={e => setFormData(p => ({ ...p, current: e.target.value }))} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm" />
            <input placeholder="ETA (e.g. Dec 2025)" value={formData.eta} onChange={e => setFormData(p => ({ ...p, eta: e.target.value }))} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm" />
          </div>
              ...
              <Card key={goal.id} className="cursor-pointer group hover:border-blue-500/50 hover:shadow-md transition-all">
                ...
                <div className="mt-4">
                  ...
                  <div className="text-right mt-2 text-xs text-slate-400">
                    {isComplete ? <span className="text-emerald-500 font-medium">Goal reached! 🎉</span> : goal.eta ? `ETA: ${goal.eta}` : ''}
                  </div>
                </div>
              </Card>
            );
